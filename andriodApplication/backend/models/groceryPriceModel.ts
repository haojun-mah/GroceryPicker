...
export async function getProductsByNames(
  ...
  try {
    ...
    for (const itemName of itemNames) {
      ...
      if (queryEmbedding === null) { // Handle case where embedding generation failed or text was empty
        console.warn(`Skipping semantic search for '${itemName}' due to failed embedding generation.`);
        continue;
      }
