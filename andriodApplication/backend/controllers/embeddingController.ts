...
if (!EMBEDDING_API_KEY) {
  console.warn("WARNING: EMBEDDING_API_KEY is not set in .env. Embedding endpoint will be insecure or unavailable.");
}
...
export const embedTextController = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!incomingApiKey || incomingApiKey !== EMBEDDING_API_KEY) {
    res.status(401).json({ statusCode: 401, message: 'Unauthorized: Invalid API Key.' });
    return;
  }
  ...
  if (typeof text !== 'string' || text.trim().length === 0) {
    res.status(400).json({ statusCode: 400, message: 'Request body must contain a non-empty string "text".' });
    return;
  }
  ...
  try {
    ...
    if (embedding === null) {
      res.status(500).json({ statusCode: 500, message: 'Failed to generate embedding.' });
      return;
    }
    ...
    res.status(500).json({ statusCode: 500, message: 'Internal server error during embedding generation.' });
  }
};