...
export const scraperUploadController = async (req: Request, res: Response): Promise<void> => {
  if (!req.headers['x-api-key'] || req.headers['x-api-key'] !== SCRAPER_UPLOAD_API_KEY) {
    res.status(401).json({ statusCode: 401, message: 'Unauthorized: Invalid Scraper API Key.' });
    return;
  }
  ...
  if (!Array.isArray(products) || products.length === 0) {
    res.status(400).json({ statusCode: 400, message: 'Request body must be a non-empty array of product data.' });
    return;
  }
  ...
  try {
    ...
    if ('statusCode' in result) {
      console.error('[Controller] Error from model ingesting scraper products:', result.message);
      res.status(result.statusCode).json(result);
      return;
    }

    console.log(`[Controller] Successfully ingested ${result.count} products from scraper.`);
    res.status(200).json({ statusCode: 200, message: `Successfully ingested ${result.count} products from scraper.` });

  } catch (error) {
    const e = error as Error;
    console.error(`[Controller Error] scraperUploadController: ${e.message}`);
    res.status(500).json({ statusCode: 500, message: 'Internal server error during scraper product processing.' });
  }
};